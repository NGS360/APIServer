...
def initiate_password_reset(session: Session, email: str) -> bool:
    ...
    if user:
        ...
        # Send reset email
        send_password_reset_email(user.email, token_str, user.full_name or user.username)
    
    # Always return True to prevent email enumeration
    return True


def complete_password_reset(session: Session, token_str: str, new_password: str) -> bool:
    """
...
def create_and_send_verification_email(session: Session, user: User) -> None:
    ...
    send_verification_email(user.email, token_str, user.full_name or user.username)
