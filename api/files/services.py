...
def upload_file(uri: str, file_content: bytes, s3_client=None) -> FileBrowserData:
    """
    Upload a file to the specified URI.
    """
    if uri.startswith("s3://"):
        return upload_file_to_s3(uri=uri, file_content=file_content, s3_client=s3_client)

    return upload_file_to_local(uri=uri, file_content=file_content)


def upload_file_to_local(
    uri: str, file_content: bytes, storage_root: str = "storage"
) -> FileBrowserData:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"Permission denied creating directory for file: {full_path.parent}",
        ) from exc
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error creating directory for file: {str(exc)}",
        ) from exc
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"Permission denied writing file: {full_path}",
        ) from exc
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error writing file: {str(exc)}",
        ) from exc
    
    # Return empty FileBrowserData to indicate success
    return FileBrowserData(folders=[], files=[])


def upload_file_to_s3(uri: str, file_content: bytes, s3_client=None) -> FileBrowserData:
    """
    Upload a file to the specified URI.
    """
    if not uri.startswith("s3://"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Only S3 URIs are supported for file upload (must start with s3://)",
        )

    if not BOTO3_AVAILABLE:
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail="S3 support not available. Install boto3 to enable file upload.",
        )

    try:
        bucket, key = _parse_s3_path(uri)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e),
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Unexpected error uploading file to S3: {str(exc)}",
        ) from exc
