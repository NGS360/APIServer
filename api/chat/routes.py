...
async def delete_all_chat_threads(
    current_user: CurrentUser, client: LangGraphDep
) -> Response:
...
async def get_chat_thread_messages(
    thread_id: str, current_user: CurrentUser, client: LangGraphDep
) -> ChatThreadMessages:
    ...
    """
    result = await services.get_thread_messages(
...
async def delete_chat_thread(
    ...
    await services.delete_thread(client, thread_id, str(current_user.id))
    return Response(status_code=204)
...
async def get_thread(thread_id: str, current_user: CurrentUser, client: LangGraphDep):
    ...
    # Raw state includes tool output and executed SQL, i.e. more than the owner
    # ever saw in the UI — so never serve it without an ownership check.
    if not await services.verify_thread_owner(
        client, thread_id, str(current_user.id)
    ):
