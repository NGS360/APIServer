...
class SequencingRun(SQLModel, table=True):
    ...
    Illumina run metadata is sourced from RunInfo.xml, which is generated by the instrument and stored with the run.
    It contains:
        - runinfo_version
        - run_id
        - run_number
        ...
        - machine_id
        - run_date
    Most of the time, the run_id contains the flowcell id, but not always. It may or may not be part of the folder name,
