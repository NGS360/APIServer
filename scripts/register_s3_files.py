...
class S3FileRegistrar:
    ...
    def extract_entity_info(self, s3_key: str, patterns: Dict[str, str]) -> Tuple[EntityType, str]:
        """
        ...
        for entity_type_str, pattern in patterns.items():
            ...
            if match:
                ...
                entity_type = EntityType.PROJECT if entity_type_str == 'project' else EntityType.RUN
    ...
    def register_s3_object(self, bucket: str, obj: dict, entity_patterns: Dict[str, str]) -> bool:
        """
        ...
        if self.dry_run:
            logger.info(f"[DRY RUN] Would register: {s3_uri} -> {entity_type.value}/{entity_id}")
        ...
        try:
            ...
            file_record.upload_date = obj.get('LastModified', datetime.now(timezone.utc))
    ...
    def scan_bucket(self, bucket: str, prefix: str = "",
...
def create_sample_config():
    ...
    logger.info("python scripts/register_s3_files.py --config s3_registration_config.json")
