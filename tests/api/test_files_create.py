...
class TestFileCreation:
    ...
    def test_create_file_at_root(self, session, s3_client, test_project):
        ...
        file_record = create_file(
            session, s3_client, file_create, file_content=b"test content"
        )
    ...
    def test_create_file_in_subdirectory(self, session, s3_client, test_project):
        ...
        file_record = create_file(
            session, s3_client, file_create, file_content=b"ATCG"
        )
        
        assert file_record.relative_path == "raw_data/sample1"
        assert "raw_data/sample1" in file_record.file_path
    
    def test_create_file_invalid_entity(self, session, s3_client):
        ...
        with pytest.raises(HTTPException, match="Project not found"):
            create_file(session, s3_client, file_create, file_content=b"test")
