...
2. ~~**Actor-field spoofing must be fixed alongside Phase 1c**~~ — **done 2026-09-04.** Both create endpoints now derive the author from the authenticated caller, and `PATCH /files/{file_id}` can no longer set it at all.

   Measured before changing anything: **every row registered through these endpoints had a null `created_by`.** No caller was passing a value, so deriving it lost no attribution and gained a real author.

   The field is still *accepted* on `FileCreate` and discarded, rather than removed. That model sets `extra="forbid"`, so deleting the field turns a request that sends the key into a 422 — and one caller registers roughly 1,500 files a week through that endpoint, while the database cannot distinguish "key absent" from "key present and null". A WARNING records any value that arrives (`event: files.created_by_discarded`), so the field can be deleted on evidence rather than on hope. `PATCH` gets no such tolerance: it is already authenticated and superuser-gated, so a 422 is the honest answer to an attempt to rewrite history.

   A side finding worth keeping: `optional_current_user` is not built on `get_current_user`, so the test client fixtures — which override only the latter — left every route using `OptionalUser` seeing an anonymous caller. That made those routes effectively untestable, and a provenance test failed for a reason unrelated to the code under test. `tests/conftest.py` now overrides both.
